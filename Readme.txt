...
For training:
On executing train.js, first of all it will fetch the train and validation data from the dataset directory and convert it to tensors of required dimensions. Then it will train the model using adam optimizer and meanSquaredError as the loss. Model will get saved after each epoch to the model directory, that can be used in the script.js.
